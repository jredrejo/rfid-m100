...
class TestRFIDReaderCommands:
    ...
    def test_send_command_success(self, mock_serial_device):
        ...
        with patch("src.rfid_reader.calculate_crc", return_value=b"\x04"), patch(
            ...
            assert result is True
            # Use .call_args_list to check all calls for the expected value
            write_calls = [
                call[0][0] for call in mock_serial_device.write.call_args_list
            ]
            assert b"\xbb\x01\x03\x00\x00\x04\x7e" in write_calls
            assert Command.TERMINATOR.value in write_calls
